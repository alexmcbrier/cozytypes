        ...
        <div id = "showSignIn" style = "padding:0rem 2rem;" >Leaderboard</div>
            ...
            <div id = "displayStats" style= "margin: 3rem;">
                <div class = "statsContainer" style = "padding: 0">
                    ...
                    <?php
                    $query = "SELECT id, MAX(wpm) AS best_wpm FROM typingtest WHERE id IS NOT NULL AND mode = 'time' AND wpm < 250 AND testTime = 15 GROUP BY id ORDER BY best_wpm DESC LIMIT 5;";
                ...
                <div class = "statsContainer" style = "padding: 0">
                    ...
                    <?php
                    $query = "SELECT id, MAX(wpm) AS best_wpm FROM typingtest WHERE id IS NOT NULL AND mode = 'time' AND wpm < 250 AND testTime = 30 GROUP BY id ORDER BY best_wpm DESC LIMIT 5;";
                    $result = $mysqli->query($query);
                    $rows = $result->fetch_all(MYSQLI_ASSOC);            
                    // Ensure there are at least 5 rows, adding empty rows if necessary
                    while (count($rows) < 5) {
